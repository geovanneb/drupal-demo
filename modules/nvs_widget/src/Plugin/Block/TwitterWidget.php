...
class TwitterWidget extends BlockBase {
  ...
  public function getTweets(){
    ...
    foreach ($my_tweets as $key => $value) {
      if(isset($my_tweets->errors)){           
          $tweetout .= '<div class="recent-tweets">Error :'. $my_tweets->errors[0]->code. ' - '. $my_tweets->errors[0]->message.'</div>';
      }else{
        //print_r($value);
        //profile_image_url
        ...
        //output custom here
        $tweetout .= '<div class="recent-tweets"><p>'.$this->makeClickableLinks($status).'</p><small class="title-2">'.$this->twitter_time($created).'</small></div>';
  ...
  function makeClickableLinks($s) {
    return preg_replace('@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?)?)@', '<a target="blank" class="blue-color" rel="nofollow" href="$1">$1</a>', $s);
  }
}
